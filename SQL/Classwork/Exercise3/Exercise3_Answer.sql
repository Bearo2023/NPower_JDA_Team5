...
SELECT SUM(AmountinUSD) FROM indian_startup_funding WHERE CityLocation = 'Bengaluru' AND AmountinUSD>380000;
...
SELECT CityLocation, AmountinUSD FROM indian_startup_funding WHERE AmountinUSD>380000 GROUP BY CityLocation;
...
SELECT StartupName, AmountinUSD FROM indian_startup_funding WHERE AmountinUSD<380000 ORDER BY StartupName DESC;
...
SELECT IndustryVertical, SUM(AmountinUSD) FROM indian_startup_funding WHERE IndustryVertical LIKE 'A%' GROUP BY IndustryVertical;
...
SELECT IndustryVertical, SUM(AmountinUSD) as Total_AmountinUSD FROM indian_startup_funding WHERE IndustryVertical LIKE 'A%' GROUP BY IndustryVertical ORDER BY Total_AmountinUSD DESC; 
--12. Write SQL query to count all the start_ups in the Education field
SELECT IndustryVertical, COUNT(*) FROM indian_startup_funding WHERE IndustryVertical = 'Education';
--13. Write SQL query to count all the start_Ups in the E-Commerce field
SELECT IndustryVertical, COUNT(*) FROM indian_startup_funding WHERE IndustryVertical in ('E-Commerce', 'E-commerce','Ecommerce');
--14. Write SQL query to count all the start_Ups in the E-Commerce field, where city location equals “Bengaluru”
SELECT IndustryVertical, COUNT(*) FROM indian_startup_funding WHERE IndustryVertical in ('E-Commerce', 'E-commerce','Ecommerce') AND CityLocation = 'Bengaluru';
...
SELECT StartupName, AmountinUSD FROM indian_startup_funding WHERE StartupName = 'Uniphore' AND AmountinUSD = (SELECT Min(AmountinUSD) FROM indian_startup_funding WHERE StartupName = 'Uniphore');
...
UPDATE indian_startup_funding SET StartupName = Upper(StartupName) WHERE AmountinUSD > 380000;
...
SELECT DISTINCT(lower(IndustryVertical)) as industry_vertical FROM indian_startup_funding;